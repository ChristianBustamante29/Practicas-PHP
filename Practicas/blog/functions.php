...
# Return: la conexion o flase si hubo un problema.
...
# Return: los post dependiendo de la apgina que estemos y cuantos post por pagina establecimos.
function obtener_post($post_por_pagina, $conexion){
    // 1.- Obtenemos la pagina actual.
    // $pagina_acutal = isset($_GET['p']) ? (int)$_GET['p']: 1;
    // Para reutilizar el codigo creamos una funcion que nos dice la pagina actual.
    
    // 2.- Determinamos dede que post se mostrara en pantalla.
    $inicio = (pagina_actual() > 1) ? pagina_actual() * $post_por_pagina - $post_por_pagina : 0;

    // 3.- Preparamos nuestra consulta trayendo informacion e indicandole desde donde y cuantas filas.
